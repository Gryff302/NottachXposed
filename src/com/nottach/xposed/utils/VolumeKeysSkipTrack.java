...
public class VolumeKeysSkipTrack {
	...
	public static void init(boolean alsoForScreenOn) {
		try {
			VolumeKeysSkipTrack.alsoForScreenOn = false; // alsoForScreenOn;

			Class<?> classPhoneWindowManager = findClass(
					"com.android.internal.policy.impl.PhoneWindowManager", null);
			XposedBridge.hookAllConstructors(classPhoneWindowManager,
					handleConstructPhoneWindowManager);

			findAndHookMethod(classPhoneWindowManager,
		...
		protected void beforeHookedMethod(MethodHookParam param)
			...
			if (!isScreenOn || alsoForScreenOn) {
				...
				if ((keyCode == KeyEvent.KEYCODE_VOLUME_DOWN || keyCode == KeyEvent.KEYCODE_VOLUME_UP)
						&& (Boolean) callMethod(param.thisObject,
								"isMusicActive") == true) {
		...
		protected void afterHookedMethod(final MethodHookParam param)
			...
			Runnable mVolumeUpLongPress = new Runnable() {
				...
				public void run() {
					...
					// for Pandora, etc...
					sendMediaButtonEvent(param.thisObject,
			...
			setAdditionalInstanceField(param.thisObject, "mVolumeUpLongPress",
					mVolumeUpLongPress);
			setAdditionalInstanceField(param.thisObject,
					"mVolumeDownLongPress", mVolumeDownLongPress);
		}
	};

	private static void sendMediaButtonEvent(Object phoneWindowManager, int code) {
		Context mContext = (Context) getObjectField(phoneWindowManager,
				"mContext");
	...
	private static void handleVolumeLongPress(Object phoneWindowManager,
			int keycode) {
		Handler mHandler = (Handler) getObjectField(phoneWindowManager,
				"mHandler");
		Runnable mVolumeUpLongPress = (Runnable) getAdditionalInstanceField(
				phoneWindowManager, "mVolumeUpLongPress");
		Runnable mVolumeDownLongPress = (Runnable) getAdditionalInstanceField(
				phoneWindowManager, "mVolumeDownLongPress");

		mHandler.postDelayed(
				keycode == KeyEvent.KEYCODE_VOLUME_UP ? mVolumeUpLongPress
						: mVolumeDownLongPress, ViewConfiguration
						.getLongPressTimeout());
	}
